...
## 23:00 验收补充（2026-09-13 晚间迭代）

在 06:00 验收态基础上，晚间继续迭代了四件事（均已提交推送，见 `git log`）：

| 变更 | 类型 | 关键点 |
| --- | --- | --- |
| **会话附件共享** | 完整性（修真实缺陷） | 此前会话里的附件只有上传者与组织管理员能下载，收件人是死链；现在参与者可下载并在「文件」页看到，非参与者 404、退出即失效、撤回后失去捷径 |
| **越权引用防护** | 安全性 | 消息只能携带自己上传的文件；读取授权要求引用消息由**文件所有者**发出。否则任何成员写一条带别人文件 id 的消息即可把别人的私有文件变成可读（已补回归测试） |
...
终态（2026-09-13 22:25）：`node scripts/acceptance.mjs` → **6/6 步**（typecheck 0 错误 · 21 文件 / 193 用例 · build · `/health` · 冒烟 27/27 · 客户端 E2E 34/34）；`git push` 已到 `origin/main`（`9b0f60f`）。
