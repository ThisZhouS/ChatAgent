...
- **发送幂等已线上补齐**：以同一 `clientMsgId` 连发两次，服务端两次返回**同一个消息 id**（实测 `189e3468-…`），会话里该文本**只有 1 条**（copies=1）——重放不会写出第二条消息。上一轮没做成的原因是我挑的会话自己不是参与者（两次都 403），不是幂等失效；换到自己参与的会话（dev 主体 7/21 个会话）后一次就通过。
...
## 澄清提问的边界（2026-09-17，P2）

- **`ask_user` 无副作用**：它只报告问题；写入会话的是服务端，任务随即进入 `waiting_input`，不会执行任何外部动作。
- **回答必须来自提问对象**：只有同一 (账号, 会话, 请求者) 的下一条消息会被当作答案（并把内容追加进该任务历史后 `resume`）；他人消息不会被采纳，也不会因此新建任务。
- **一次一个问题**：同一组合只允许一个未回答问题，避免多个开放问题导致答案归属歧义；问题随任务持久化，重启后仍在等待。
- **澄清优先于审批**：两种等待分开判定，避免把「需要补充信息」误记成「等待审批」。

## 会话外观（2026-09-17，P2）

- **闭集而非样式串**：背景只接受预设 id（小写字母开头）或 #rrggbb，其余（url(...)、分号拼接、大小写变体、多余字段）一律 400；渲染端只把它们映射为普通颜色，不使用背景图。理由：该值会被每个客户端渲染，样式串是 CSS 注入面。
- **不改变消息对比度**：气泡保留自身实色背景，房间背景只影响房间底色；深色房间自动切浅色文字，既有对比度检查（4.5:1）继续适用于气泡文本。
- **无权限提升**：外观与群名同级，任何参与者可改；它不携带权限、不参与授权判定。

## 会话别名（2026-09-17，P2）

- **别名是查看者私有数据**：存在该成员自己的已读行上（`ReadStateStore`），只随他自己的会话摘要返回；不能给别人改名，也不会让任何人知道别人怎么称呼自己。
- **只能标注本会话成员**：给会话外成员起别名会被 400 拒绝（`unknown_member`），避免产生永远渲染不出的孤儿标签；非参与者对整条接口 404。
- **有界**：标签 ≤32 字、整表 ≤200 项、多余字段拒绝；空串清除，因此「无别名」只有一种表示。

## 事件游标（2026-09-18，ADR-0004）

- **不能把“无法满足的游标”当成“没有新事件”**：可回放范围之外的游标会触发 `event: resync`，客户端必须重载；静默返回空列表等于让客户端永久少一段消息。
- **回放仍逐条鉴权**：截断通知只说明“你旧了”，不放宽任何授权判定。
- **存储边界**：单进程单写者、原子替换、fsync 后返回；跨 store 的原子性靠顺序 + 幂等键，不是事务；迁移触发条件见 ADR-0004。

## 表情与贴纸（2026-09-18，P2）

- **贴纸是闭集**：消息只带目录内的 id，服务端校验（未知 id 400），客户端用自带资源渲染；不存在「贴纸 URL」这种字段，因此贴纸不能被用来指向外部内容或做 SSRF/像素追踪。
- **表情就是文字**：插入的是普通字符，不引入新的渲染路径（沿用既有文本渲染，无 HTML 注入面）。

## 成员助手偏好（2026-09-22，产品决定 5）

- **只能改自己，而且是结构性的**：`GET/PATCH /api/preferences` 都**不接受成员 id**，因此不存在「传别人的 id」这条路径可被越权；客户端尝试带 `memberId` 会被 `.strict()` schema 判 400（不是被静默忽略）。用例：`preferences.test.ts`。
- **越界是拒绝而不是夹紧**：只有 1–200 的整数被接受，其余（0、201、小数、字符串、空 patch）一律 400 且**什么都不写**——静默夹紧会让「设了但没生效」看起来像生效。
- **不扩大读取面**：偏好只决定「这次请求带多少历史进模型」（上限 200 条，仍在原有 `CHATAGENT_AGENT_CONTEXT_MESSAGES` 语义内），不改变任何授权判定、不让成员看到别人的会话；存储行只含成员 id 与两个整数。
- **偏好读取失败不阻断工作**：投喂时查偏好抛错只记 warning 并回落部署默认值（用例 `agent-intake.test.ts`「偏好读取失败仍以默认投喂」），避免「配置文件坏了 → 助手不回话」。
- **可追溯**：PATCH 写审计 `member.preferences_updated`（只记改动前后的数字，不记内容）。

## 谁能看到谁（2026-09-22，产品决定 1C-(a)）

- **可见性在发现层，不在沟通层**：联系人列表（`GET /api/contacts`）只含「有关系记录的人」——好友、任一方向的待处理申请、被拉黑或起过私有备注的人——以及 AI 账号；全组织仍可通过 `GET /api/members` 列出与搜索。**不加好友也能单聊、拉群、@、收发消息**（唯一一次把单聊挂到友谊上的试做让 35 个用例失败并回退；现在这条语义有专门的回归用例）。
- **判据是关系行/待处理申请，不是渲染状态**：备注与拉黑本身是关系记录，必须仍可见，否则「你给某人起的备注/拉黑」将无法再被编辑或撤销。
- **在线状态只对本人与好友可见**，三处一致：`/api/contacts` 的 `online`、`/api/members` 的 `online`、`/api/presence` 的列表。只收窄前两者会留下同一个问题的第三个答案，等于没做。
- **目录不返回敏感字段**：只有成员 id、显示名、组织、roles 与（仅好友的）在线状态；令牌从不出现。roles 对所有人保留是**有意的**——管理台 `MembersView` 就从这个接口读角色，剥掉会让管理员看不到成员角色。
- 证据：`contact-visibility.test.ts` 4 例（含陌生人视角的目录/presence 断言与「非好友仍可单聊、拉群」的回归）；`native-chat.test.ts`、`friends.test.ts` 中被改写的 4 处断言都是**旧「全组织可见」假设**，改写理由逐条写在该切片文档 §3.20 里。

## 个人 ID / handle（2026-09-22，产品决定 8B）

- **只能改自己**：`PATCH /api/auth/handle` 的请求体里只有 handle，没有成员 id；服务端从会话取主体，客户端连"改别人"都表达不出来。
- **防冒充**：组织内唯一（409），保留词拒绝（`admin`/`owner`/`system`/`ai` 等，且先规范化再校验，`Admin` 与 `admin` 是同一个名字）；改名后旧名进入保留期（默认 90 天），期间他人取用返回 409，**本人可取回**，到期自动释放——既不让别人捡走同事刚用过的名字，也不永久占坑。
- **不构成权限**：handle 只是可搜索的展示名。它不参与任何授权判定（授权仍看成员 id/角色/关系），改 handle 不改变任何可见性或能力；审计行 `member.handle_set` 只记 `旧->新`，不含凭据。
- **有界与可迁移**：`data/members.json` 从裸数组改为 `{members, retiredHandles}`，两种形态都能读，过期保留项在读取时丢弃，不会随改名史无限增长；格式与保留词规则在契约里定义一次，客户端与服务端共用，避免"UI 拦得住、API 拦不住"。
- 证据：`handles.test.ts` 6 例（含跨大小写重名、冷却 429、保留期内他人 409 而本人可取回、0 天保留即刻释放）。
