...
6. 设置页 →「我的助手偏好」：两个数字（每次请求带入的历史条数、澄清补充后保留的条数，范围 1–200）。默认显示的是**当前生效值**（20 / 50，未设置过即部署默认）；改一个数字点「保存」，提示「已保存」且「当前生效」随之更新。这只影响**你自己**发出的请求；把值改成 0/201 会被服务端拒绝（400），页面显示错误且「当前生效」保持旧值——夹紧或假装成功都是要避免的。
