...
UI 手工看三眼：① 联系人在线**绿点**（只对你和你的好友显示）与 AI 会话里生成文件的**📎 下载**；
...
- 联系人列表：只列出**与你有关系的人**——好友、正在申请或被申请的人、你拉黑或起过备注的人，以及 AI 账号；没有任何关系的同事不出现在这里（产品决定 1C-(a)）。要找他们，用联系人卡片上方的「搜索组织目录（添加同事）」：搜到后只显示姓名与「加好友」，**不显示在线状态**。
- 在线绿点：只对**你自己和你的好友**显示（副标题「· 在线」），陌生人看不到你的在线状态，`GET /api/presence` 同样只返回自己与好友。
- 与没有加好友的同事**照常可以**开单聊、拉群、@ 与收发消息：可见性只作用于「谁能看到谁」，不改变「谁能和谁说话」。
...
6. 设置页 →「我的个人 ID」：默认显示服务端给你的派生名字（老成员首次打开时自动分配，不是空白）。改成 `alice.wang` 保存 → 提示已保存、「当前：@alice.wang」；同事在目录里搜 `alice.wang` 就能找到你。试 `admin`（保留词）或 `ab`（太短）→ 保存被拒并显示服务端原因；再改第二次会撞冷却（默认 30 天，`CHATAGENT_HANDLE_CHANGE_COOLDOWN_DAYS=0` 可关闭）；改掉的名字在保留期内（默认 90 天）别人取用会被拒，但你自己可以拿回来。
7. 设置页 →「我的助手偏好」：两个数字（每次请求带入的历史条数、澄清补充后保留的条数，范围 1–200）。默认显示的是**当前生效值**（20 / 50，未设置过即部署默认）；改一个数字点「保存」，提示「已保存」且「当前生效」随之更新。这只影响**你自己**发出的请求；把值改成 0/201 会被服务端拒绝（400），页面显示错误且「当前生效」保持旧值——夹紧或假装成功都是要避免的。
