...
- `pnpm typecheck` / `pnpm build` 通过；`pnpm test` **21 文件 / 193 用例**（服务端与包 168 + Web 25）全绿；冒烟 27/27；客户端 E2E **34/34**。
...
- 验证：`pnpm typecheck` / `pnpm build` 通过；`pnpm test` **21 文件 / 178 用例**；冒烟 **27/27**（新增撤回/回执/导出/会话/在线等 11 项）；客户端 E2E **34/34**。
...
- 客户端 E2E 在**空数据目录 + production 档位**的全新实例上 34/34（顺带修掉脚本对「全新部署无会话」的假设）。
...
- 验证：`pnpm test` **21 文件 / 181 用例**、冒烟 27/27、客户端 E2E 34/34。

### 附件共享与消息转发（22:00）

- 修真实缺陷：会话附件此前只有上传者与管理员能下载（收件人是死链）。现在会话参与者可下载，且新增两道越权防护（只能附带自己上传的文件；读取授权要求引用消息由文件所有者发出）。
- 新增消息转发（不转发 AI、撤回不可复活、审计 `message.forwarded`，前端「转发」+ 提示）。
- 验收脚本加固：E2E 头等待改为等待会话标题（客户端会先自动选中别的会话），截图失败不再让整轮失败。
