...
## ⑱ 2026-09-17 下午：回执失败分级（Gate 7A.2 剩余第 3 项）

- 问题：设置页对回执失败只会说“联网后自动重试”，但 4xx 是服务端拒收，等多久都不会好。
- 做法：失败分级 `server_rejected | server_error | network` 并随状态文件持久化；拒收后 15 分钟退避（连手动触发也跳过，避免对已被拒绝的请求反复打点），5xx/网络仍走指数退避；回执永不丢弃；设置页按分级给出可操作的话术，无异常时不显示。
- 结果：`receipt-sync.test.mjs` 13 → 16 例，web 新增文案分级用例，根套件 33 文件 / 324 用例，真实 Electron 回执检查 21/21。

## ⑲ 2026-09-17 傍晚：保留策略可见性、打包 E2E 与 Electron 升级彩排

- 保留策略：离线工作台补齐保留/授权/回执三类提示（同一份 `status()` 载荷），检查脚本 11 → 13 项（含反向断言：干净运行不得报警）。
- 打包：`electron-builder` 离线重打包成功；重建 server dist 后跑打包后客户端 E2E **38/38**。顺手修掉 E2E 竞态（`:loading` 吞点击 + 单次气泡检查即重试 → 消息丢失假象），改为等按钮可点/等气泡/不重复输入，并新增“文档请求已发出”断言与失败诊断。
- 升级：39.8.10 已 EOL（2026-05-05），目标 44.4.1；用缓存里的 42.4.1 运行时做彩排，7 个桌面壳检查 + 打包后 E2E 全绿；新增 `CHATAGENT_ELECTRON_BIN`、`ui-e2e --exe`。换版本本身受限于无 registry 网络，记录为 BLOCKED，详见新文档。
## ⑳ 2026-09-17 夜间：审查入库 + 令牌卫生 + 推送

- 用户要求「重新审计，并给出到完整可应用的缺口与方向」，审查报告（GAP-01~08 + M1–M4）与请求留痕入库；本轮不改业务代码。
- 审查顺带发现 `scripts/ui-e2e.mjs` 内默认令牌可用：改为 env/Temp 解析 + `scripts/ensure-e2e-member.mjs` 自动签发本机 `e2e_local`，两条路径均以 38/38 复验；旧令牌留在 git 历史中，公开仓库需轮换。
- 推送 `main` 到 GitHub（SSH 可达）。