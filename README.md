...
**当前状态（2026-09-13 06:00 验收态）**：8 个页面全部可用，`node scripts/acceptance.mjs` 一条命令 6/6 步通过（类型检查 0 错误 · 21 文件 / 186 用例 · 构建 · 服务端 `/health` · 接口冒烟 27/27 · 真实客户端 E2E 33/33）；安全面经**五轮独立对抗性复核**逐条复现并修复（含 1 个 P0 会话劫持）。详见 [`docs/acceptance-report.md`](docs/acceptance-report.md)。
...
pnpm test                          # 单元/集成测试：21 文件 / 186 用例
...
会话头部「导出记录」可把会话导出为 Word 记录（撤回内容只留 `[已撤回]` 占位）。
